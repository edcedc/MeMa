    ...
    protected void onBindViewHolde(RecyclerView.ViewHolder holder, int position) {
        ...
        if (bean.getRegionLevel() >= 3){
            if (mPosition == position){
                viewHolder.tv_locate.setCompoundDrawablesWithIntrinsicBounds(null,null,
        ...
        viewHolder.itemView.setOnClickListener(view -> {
            if (listener != null)listener.onClick(bean.getRegionId(), bean.getRegionName(), bean.getRegionLevel(), position);
        });
    }

    private OnClickListener listener;
    public interface OnClickListener{
        void onClick(String parentId, String address, int regionLevel, int position);
    }
